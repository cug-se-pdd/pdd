...
class AdminEnrollService extends BaseProjectAdminService {
    ...
    try {
			...
			const result = await db.collection('enroll').add({
				data: {
					title,
					cateId,
					cateName,
					...
					status: 1 // 假设1表示'活跃'状态，可以根据实际业务需求调整*/
				}
			});
			// 数据添加成功后，返回成功消息
			return { success: true, id: result._id, message: "登记信息添加成功" };
		} catch (error) {
			// 捕获并处理可能发生的错误
			console.error('插入登记数据失败:', error);
			throw new Error('操作失败，数据库写入错误');
		}
	}
